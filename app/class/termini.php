...
class zakazaniTermini {
    public function fetch_all_zakazani() {
        global $pdo;
        $query = $pdo->prepare("SELECT * FROM termini WHERE termini_status = 'Zakazan' ORDER BY termini_datum, termini_vrijeme ASC");
        $query->execute();
        return $query->fetchAll();
    }
}
...
//GET ALL TERMINI IZMEĐU DATUMA
class zakazaniBetween {
    public function fetch_all_sedmica($ponedjeljak, $subota) {
        global $pdo;
        $query = $pdo->prepare("SELECT * FROM termini WHERE termini_datum BETWEEN :start AND :end AND termini_status = 'Zakazan' ORDER BY termini_datum, termini_vrijeme ASC");
